...
def run(problem, params):
    ...
    for run in range(num_of_runs):
        ...
        for it in range(max_gen):
            ...
            # Store best cost
            bestcost[it] = round(bestsol.cost,1)
            X = round(bestsol.position[0],1)
            Y = round(bestsol.position[1],1)


            # Show iteration information
            if it == 29: #Get result from last generation
                #print("Generation {}:, Best Cost {}:, Min X is {}:, Min Y is {}".format(it, bestcost[it],X,Y))
                print("Run {}:, Best Cost {}:, Min X is {}:, Min Y is {}".format(run, bestcost[it],X,Y))
            #Results
